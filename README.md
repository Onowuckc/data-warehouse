...
This section guides you through installing the necessary software and running the project using dummy CSV data.

### 1. Install Microsoft SQL Server Express Edition
...
- Place your dummy CSV files in the folders expected by the `bronze.load_bronze` stored procedure or update the file paths in `scripts/bronze/proc_load_bronze.sql` accordingly.  
- Open SSMS and connect to your SQL Server instance.

### 4. Run the Project Scripts

- Execute `scripts/init_database.sql` to create the `DataWarehouse` database and the `bronze`, `silver`, and `gold` schemas.  
- Execute the stored procedure `bronze.load_bronze` to load raw data from CSV files into the bronze tables:  
  ```sql
  EXEC bronze.load_bronze;
  ```  
- Execute the stored procedure `silver.load_silver` to transform and cleanse data into the silver tables:  
  ```sql
  EXEC silver.load_silver;
  ```  
- Query the gold layer views for analytics and reporting, for example:  
  ```sql
  SELECT * FROM gold.dim_customers;
  SELECT * FROM gold.fact_sales WHERE order_date > '2023-01-01';
  ```

### 5. Optional: Data Quality Validation

- Run the SQL scripts in the `tests` folder to perform data quality checks on the silver and gold layers.
...
- ✅ **ChatGPT:** [Course Link](https://www.youtube.com/watch?v=LJLNfei4i-c) | [Download Materials](https://datawithbaraa.substack.com/p/access-to-project-materials)  
